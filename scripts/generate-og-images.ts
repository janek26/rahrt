...
async function generateOGImage() {
  try {
    ...
    // Avatar is 180px tall, so we need proper spacing between all elements
    const avatarY = 100;
    const nameY = 350; // Balanced gap from avatar
    const subtitleY = 410; // Balanced gap from name
    const tagsY = 475; // Balanced gap from subtitle
    const tagHeight = 36;
    ...
    // Save as single shared image (used by both Open Graph and Twitter)
    const outputPath = `${IMAGES_DIR}/og-image.png`;
    await writeFile(outputPath, finalImage);
    console.log(
      "✅ Generated images/og-image.png (shared for Open Graph and Twitter)"
    );
